...
The gate guarantees exactly one thing: **every required item is present** — you never build an
answer over a silent hole. That is *fabrication* (computing on data that isn't there), and it
becomes impossible. It does **not** guarantee you used the *right* present item, nor that a
present item *means* what you assert.

- ✅ **Guaranteed (deterministic):** the required keys/IDs/fields are all in the context, or you
  abstain with the exact gap. No answer is ever built from missing data.
- ❌ **NOT guaranteed (still semantic):** **misselection** — grounding on a present-but-*wrong*
  item (the right value is in the context but you used a different one that also exists), or a
  present item being *irrelevant* / meaning the opposite of what you claim. The gate checks
  *presence*, not *correctness of choice* or *meaning*.

So passing the gate means **"no answer was built from missing data,"** not **"the answer is
right."** For the choice/meaning half you are back in semantic territory — verify it separately
or flag it for a human. (Concretely: a citation-existence check catches *fabricated* cases
deterministically, but not a *real* case cited for the wrong holding — that is misselection.)
...
- ❌ Declaring the required set loosely ("the relevant IDs") instead of concretely
  (`set(range(200, 251))`).
- ❌ Building `present` from your own narration of what you did ("I already extracted all of
  them", "I placed every object") instead of from the actual source. `present` must come from
  running code over the real context / querying the real store — never from your memory of
  having handled it. Self-report reintroduces exactly the judgment the gate removes.
- ❌ Treating a PASS as "the answer is correct." A PASS means "nothing required is missing" —
  it says nothing about whether you chose the right present item (see *misselection* above).
