...
- **The frontier itself is now mapped.** A pilot cartography of real decisions
  ([OPRATE.md](OPRATE.md)): ~72–75% are operationalizable *now* (code or execution), ~38% of the
  apparent "semantic valley" is unwritten contracts (spec debt), and the genuine residue is
  ~15% — whose routing cannot be made deterministic (it is observer-relative), but whose failure
  modes are few, named, and forced into the open by a declaration discipline (CHECK/ASSUMES),
  packaged as a skill and ablated against a real agent: it buys the floor and reproducibility,
  not the ceiling.
...
| [OPRATE.md](OPRATE.md) | the operationalization rate: mapping the decidable-vs-semantic frontier, and the skill ablated against a real agent |
